...
const modifyTask = async (tasks) => {
  await fetch(
    `https://jsonrepository-production.up.railway.app/tasks/${tasks.id}`,
    {
      method: "PUT",
      body: JSON.stringify({
        email: tasks.eemail,
...
const editTask = async (id) => {
  const apiResponse = await fetch(
    `https://jsonrepository-production.up.railway.app/tasks/${id}`
  );
...
const buttonADD = async () => {
  const apiResponse = await fetch(
    `https://jsonrepository-production.up.railway.app/tasks/`
  );
...
const buttonAddUser = async () => {
  ...
  const newUserPasswordConfirming =
    formAdduser.elements["newUserPasswordConfirm"].value;
  ...
  if (findEmail === undefined) {
    if (
      newUserpassword === newUserPasswordConfirming &&
      newUserPasswordConfirming != null
    ) {
      await fetch("https://jsonrepository-production.up.railway.app/tasks", {
        method: "POST",
        headers: {
...
const getTasks = async () => {
  ...
  let tasks = user.filter(function (findTasks) {
    if (
      user[currentUSER - 1].email === findTasks.email &&
      findTasks.description != undefined
    ) {
      return true;
    }
    return false;
